...
(define-foreign-type term-criteria-struct ()
  ((garbage-collect  :reader garbage-collect :initform nil :initarg 
                     :garbage-collect))
  (:actual-type :pointer)
  (:simple-parser term-criteria-struct))


(defclass cv-cv-term-criteria ()
  ((c-pointer :reader c-pointer :initarg :c-pointer)))


(defmethod translate-to-foreign ((lisp-value cv-cv-term-criteria) (c-type term-criteria-struct))
  (values  (c-pointer lisp-value) lisp-value))


(defmethod translate-from-foreign (c-pointer (c-type term-criteria-struct))
  (let ((matrix  (make-instance 'cv-cv-term-criteria :c-pointer c-pointer)))
    (when (garbage-collect c-type)
      (tg:finalize matrix (lambda () (del-cv-mat c-pointer))))
    matrix))
...
(defmethod translate-from-foreign (c-pointer (c-type svm-params))
  (let ((svm-params  (make-instance 'cv-svm-params :c-pointer c-pointer)))
    (when (garbage-collect c-type)
      (tg:finalize svm-params (lambda () (del-svm-params c-pointer))))
    svm-params))
